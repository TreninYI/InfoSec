...
Я сразу увидел подсказку, требуется проверить поддомен `task.cyber-ed.ru`.
...
Далее проверил, что войти под учетной записью админ получилось, но так же столкнулся с трехзначным кодом. По той же схеме подобрал трехзначный код и попал на сервер, где лежал флаг.
...
Для выполнения данного задания требуется иметь базовые знания таблиц SQL.
...
С помощью BurpSuite нашел запрос, отправил его в репитер, затем вставил предложение в поле телефона.
...
Для начала пришлось настроить сеть между машинами Kali и Windows
